...
public class CourseOrderServiceImpl implements CourseOrderService {
    ...
    public List<CourseOrder> listOrdersByCourseId(Integer cid) {
        ...
        if ( courseOrders == null || courseOrders.isEmpty() ) throw new OrderNotFound(String.format("failure to find the courseOrders by course id:%d",cid));
        return courseOrders;
    }
    ...
    public List<CourseOrder> listOrdersByLoginName(String login_name) {
        ...
        if ( courseOrders == null || courseOrders.isEmpty() ) throw new OrderNotFound(String.format("failure to find the courseOrders by login_name:%s",login_name));
        return courseOrders;
    }

    public CourseOrder get(Integer oid) {
        CourseOrder courseOrder = courseOrderRepository.get(oid);
        if (courseOrder == null) throw new OrderNotFound(String.format("failure to find the courseOrder by oid:%d",oid));
        return courseOrder;
    }
